...
export class LectureNoteService {
  ...
  private readonly section = 'lecture_notes';
  ...
  constructor(private http: HttpClient, private imageUploadService: ImageUploadService,
    ...
    this.baseUrl = this.baseUrl + '/api/lecture-notes';
  }
  ...
  uploadDocument(data: FormData): Observable<Document> {
    ...
    const classLevel = data.get('classLevel') as string;
    const file = data.get('file') as File;

    if (!file || !subject || !size || !date || !classLevel) {
      console.error('[LectureNotes] Missing required fields in FormData');
    ...
    return this.imageUploadService.uploadFile(file, this.section).pipe(
      switchMap((uploadResponse) => {
        if (!uploadResponse?.url) {
          console.error('[LectureNotes] File upload failed');
      ...
      catchError((err) => {
        console.error('[LectureNotes] Upload error:', err);
